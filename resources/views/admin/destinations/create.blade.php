            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.destinations.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                ...
                                <input type="number" id="entrance_fee" name="entrance_fee"
                                    value="{{ old('entrance_fee') }}" class="form-control" step="0.01">
                                @error('entrance_fee')
                                    <div class="text-danger mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="social_media" name="social_media" class="form-control" rows="3">{{ old('social_media') }}</textarea>
                        @error('social_media')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <script>
                            var map = L.map('map').setView([18.356104, 121.639880

                            ], 13);
                            L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
                                attribution: '© OpenStreetMap contributors'
                            }).addTo(map);
                            var marker = L.marker([18.356104, 121.639880], {
                                draggable: true
                            }).addTo(map);
